...
export const EntryForm = ({
  ...
  return <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="mt-4">
        {activeInput === "text" && <div className="space-y-2">
            <div className="flex items-center gap-1 p-2 bg-zinc-800/50 border border-zinc-700/50 rounded-t-xl">
              <Button type="button" size="sm" variant="ghost" onClick={() => editor?.chain().focus().toggleBold().run()} className={cn("p-2 h-8 w-8", editor?.isActive('bold') ? 'bg-zinc-700/50' : '')}>
                <Bold className="h-4 w-4" />
              </Button>
              <Button type="button" size="sm" variant="ghost" onClick={() => editor?.chain().focus().toggleItalic().run()} className={cn("p-2 h-8 w-8", editor?.isActive('italic') ? 'bg-zinc-700/50' : '')}>
                <Italic className="h-4 w-4" />
              </Button>
              <Button type="button" size="sm" variant="ghost" onClick={() => editor?.chain().focus().toggleBulletList().run()} className={cn("p-2 h-8 w-8", editor?.isActive('bulletList') ? 'bg-zinc-700/50' : '')}>
                <List className="h-4 w-4" />
              </Button>
              <Button type="button" size="sm" variant="ghost" onClick={() => editor?.chain().focus().toggleOrderedList().run()} className={cn("p-2 h-8 w-8", editor?.isActive('orderedList') ? 'bg-zinc-700/50' : '')}>
              ...
              <div className="ml-auto flex items-center gap-1">
                ...
                <Button type="button" size="sm" variant="ghost" onClick={() => editor?.chain().focus().redo().run()} className="p-2 h-8 w-8">
                  <Redo className="h-4 w-4" />
                </Button>
              </div>
            </div>
            <div className="min-h-[300px] my-0 ">
              <EditorContent editor={editor} className="h-full p-4 bg-zinc-900/50 border border-zinc-700/50 rounded-b-xl text-zinc-100 focus-within:border-zinc-600 transition-colors prose prose-invert max-w-none prose-sm overflow-y-auto py-[45px]" />
            </div>
          </div>}
        
        {activeInput === "url" && <div className="space-y-2 bg-zinc-900/30 p-4 rounded-xl border border-zinc-800/50">
            <p className="text-sm text-zinc-400">
              Enter a URL to analyze its content and save key information.
            </p>
            <Input type="url" value={url} onChange={e => setUrl(e.target.value)} placeholder="https://..." className="bg-zinc-900/50 border-zinc-700/50 text-zinc-100 placeholder:text-zinc-500 focus-visible:ring-zinc-700/50" />
          </div>}

        {activeInput === "image" && <div className="space-y-2 bg-zinc-900/30 p-4 rounded-xl border border-zinc-800/50">
            <p className="text-sm text-zinc-400">
